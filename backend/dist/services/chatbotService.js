...
class ChatbotService {
    ...
    async searchDatabaseWithTimeout(question) {
        const lowerQuestion = question.toLowerCase();
        // Only use keyword search for specific statistics questions, not general location questions
        const isStatisticsQuestion = (lowerQuestion.includes('statistics') ||
    ...
    async searchDatabaseRAG(question) {
        try {
            // Generate embedding for the question
            const { embedding } = await embeddingService_1.embeddingService.generateEmbedding(question);
            // Use the stored procedure for vector similarity search
            const result = await databaseService_1.db.query(`SELECT * FROM search_knowledge_base($1::vector, $2, $3)`, [JSON.stringify(embedding), 0.7, 10] // similarity threshold: 0.7, limit: 10 results
            );
    ...
    async fallbackKeywordSearch(question) {
        console.log('⚠️  Using fallback keyword search');
        const searchTerm = `%${question}%`;
        const contexts = [];
        try {
            // Search rights_guide
            const rightsResults = await databaseService_1.db.query(`SELECT id, question, answer, law_ref
         FROM rights_guide
         ...
         LIMIT 3`, [searchTerm]);
            ...
            // Search FAQ
            const faqResults = await databaseService_1.db.query(`SELECT id, question, answer
         FROM faq
         WHERE question ILIKE $1 OR answer ILIKE $1
         LIMIT 3`, [searchTerm]);
            ...
            const lawResults = await databaseService_1.db.query(`SELECT id, title, section, content
         ...
         LIMIT 2`, [searchTerm]);
            ...
            try {
                const stateResults = await databaseService_1.db.query(`SELECT state_code, state_name_en, migrant_number, risk_level,
                  manuf_perc_in_state, const_perc_in_state, agric_percent_in_state
           FROM states
           WHERE state_name_en ILIKE $1 OR $1 ILIKE '%' || state_name_en || '%'
           LIMIT 5`, [searchTerm]);
                stateResults.rows.forEach((row) => {
                    contexts.push({
                        id: row.state_code,
                ...
                if (stateResults.rows.length === 0 &&
                    ...
                    const allStatesResults = await databaseService_1.db.query(`SELECT state_name_en, migrant_number, risk_level
             FROM states
             ORDER BY migrant_number DESC
             LIMIT 10`);
                    allStatesResults.rows.forEach((row) => {
                        contexts.push({
                            id: 0,
                ...
                console.log('States table query failed, using insights API instead:', error.message);
                ...
                try {
                    ...
                    const matchingStates = insightsData.rows.filter((state) => state.state_name_en.toLowerCase().includes(searchTerm.replace(/%/g, '').toLowerCase()) ||
                    ...
                    console.log('Insights API fallback also failed:', apiError.message);
                }
            }
            return contexts;
        }
...
NEVER use markdown formatting or citations.`;
    ...
    async chat(question, language = 'en') {
        ...
        if (contexts.length > 0) {
            const contextText = contexts.map((ctx, idx) => `[${idx + 1}] ${ctx.content}${ctx.reference ? ` [ref:${ctx.reference}]` : ''}`).join('\n\n');
            messages.push({
                role: 'system',
    ...
    generateFallbackAnswer(question, contexts, language) {
        const langMap = {
            'en': 'English',
            'ms': 'Bahasa Malaysia',
            'ne': 'Nepali',
    ...
    async calculateWage(monthlySalary, overtimeHours = 0) {
        const steps = [];
        // Daily wage
    ...
    async saveConversation(sessionId, question, response, language) {
        try {
            await databaseService_1.db.query(`INSERT INTO chatbot_conversations
