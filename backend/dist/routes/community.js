...
router.get('/organizations', async (req, res) => {
    try {
        ...
        let rows = await databaseService_1.db.getOrganizationsData();
        ...
        if (category && category !== 'All Help') {
            rows = rows.filter(org => org.service_type?.toLowerCase().includes(category.toLowerCase()));
        }
        // Filter by search term if provided
        if (search) {
            const searchTerm = search.toLowerCase();
            rows = rows.filter(org => org.org_name?.toLowerCase().includes(searchTerm) ||
...
router.get('/stories', async (req, res) => {
    try {
        const { category, search, lang = 'en' } = req.query;
        let rows = await databaseService_1.db.getSurvivorStoriesData();
        // Map to appropriate language fields
        const languageCode = lang;
        rows = rows.map(story => {
        ...
        if (category && category !== 'All Stories') {
            rows = rows.filter(story => story.theme?.toLowerCase().includes(category.toLowerCase()));
        }
        // Filter by search term if provided
        if (search) {
            const searchTerm = search.toLowerCase();
            rows = rows.filter(story => story.story_title_en?.toLowerCase().includes(searchTerm) ||
...
router.get('/resources', async (req, res) => {
    try {
        const { category, search, lang = 'en' } = req.query;
        let rows = await databaseService_1.db.getPracticalGuidesData();
        // Map to appropriate language fields
        const languageCode = lang;
        rows = rows.map(guide => {
        ...
        if (category && category !== 'All Resources') {
            rows = rows.filter(guide => guide.category_name?.toLowerCase().includes(category.toLowerCase()));
        }
        // Filter by search term if provided
        if (search) {
            const searchTerm = search.toLowerCase();
            rows = rows.filter(guide => guide.guide_topic_name?.toLowerCase().includes(searchTerm) ||
...
router.get('/organizations/:id', async (req, res) => {
    try {
      ...
      WHERE org_id = $1
    `;
        const result = await databaseService_1.db.query(query, [id]);
...
router.get('/stats', async (req, res) => {
    try {
        const [orgResult, storyResult, resourceResult] = await Promise.all([
            databaseService_1.db.query('SELECT COUNT(*) as count FROM organization'),
            databaseService_1.db.query('SELECT COUNT(*) as count FROM survivor_story'),
            databaseService_1.db.query('SELECT COUNT(*) as count FROM comm_practical_guide')
        ]);
        res.json({
